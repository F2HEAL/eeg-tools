...
def save_psd_figure(psd_obj, pdf, title=None, dpi=90):
    ...
    # Decimate frequency axis if very dense
    max_freq_points = 2000
    step = max(1, psd.shape[1] // max_freq_points)
    freqs_dec = freqs[::step]
    psd_dec = psd[:, ::step]

    fig, ax = plt.subplots(figsize=(10, 6))
    for ch in range(psd_dec.shape[0]):
        ax.plot(freqs_dec, 10 * np.log10(psd_dec[ch] + 1e-20), color='lightgray', linewidth=0.5, rasterized=True)
    mean_psd = np.mean(psd_dec, axis=0)
    ax.plot(freqs_dec, 10 * np.log10(mean_psd + 1e-20), color='red', linewidth=1.5, label='Mean')
...
def main():
    ...
    with PdfPages(out_pdf) as pdf:
        ...
        save_psd_figure(psd_obj_all, pdf, title=os.path.basename(args.file) + " - PSD (0-500 Hz)", dpi=image_dpi)
        ...
        plot_spectrograms_mne(raw, filename=args.file, pdf=pdf, fmin=0, fmax=500, dpi=image_dpi)
